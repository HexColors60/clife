...
int main(int argc, char *argv[]) {
    ...
    for(;;) { // Loop until cexit()==0
        ...
        if(!strcmp(input, ";dungeon")) { detStruct(loc, country, 2, &data); plr.gold = data.gold; }
        if(!strcmp(input, ";umsehen")) { detStruct(loc, country, 0, &data); plr.gold = data.gold; } 
        if(!strcmp(input, ";de")) { detStruct(loc, country, 1, &data); plr.gold = data.gold; }
